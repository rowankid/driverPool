...
public class DriverPool {
    	...
    	//if driver is null, the driver need to be created
            ...
            if (this.size.get() < poolProperties.getMaxSize()) {
                if (size.addAndGet(1) > getPoolProperties().getMaxSize()) {
                	...
                	System.out.println("size : " + this.size.get() + ", max size : " + poolProperties.getMaxSize());
                	driver = createDriver();
                	driver.setTimestamp(System.currentTimeMillis());
            		this.borrowedCount.incrementAndGet();
            		this.busy.put(driver);
    ...
    public void release(PooledDriver driver) throws PoolException, InterruptedException {
        ...
        try {
            ...
            this.busy.remove(driver);
        } finally {
        	driver.unlock();
        }
    }
    
    protected void discardDriver(PooledDriver driver) {
    	driver.setDiscarded(true);
    	driver.setHandler(null);
